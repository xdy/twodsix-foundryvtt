/** @typedef {import("../../entities/_module.mjs").TwodsixCombatant} TwodsixCombatant */
/** @typedef {import("../../entities/TwodsixCombat").default} TwodsixCombat */
...
export default class TwodsixCombatTracker extends foundry.applications.sidebar.tabs.CombatTracker {
  ...
  static #canUserControlCombatant(combatant) {
    if (!combatant?.actor) {
      return false;
    }
   ...
   * @*/
  _getCombatantActionData(combatant, combat) {
   ...
   * @*/
  _renderCombatantActionIndicators(combat) {
    ...
    combatantElements.forEach((element) => {
      ...
      if (combatant.usesThrustCounter?.()) {
        ...
        Object.entries(actionData).forEach(([actionType, data]) => {
          actionWrapper.appendChild(
            this._createActionControl(actionType, data, combatantId, canControl)
          );
        });
      }
