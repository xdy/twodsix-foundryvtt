...
export class AbstractTwodsixActorSheet extends ActorSheet {
          ...
          yes: async () => {
            ...
            if (ownedItem.type === "consumable") {
              selectedActor?.items.filter(i => i.type !== "skills" && i.type !== "trait").forEach(async (i) => {
                const consumablesList = i.system.consumables;
                let usedForAttack = i.system.useConsumableForAttack;
                if (consumablesList != undefined) {
                  if (consumablesList.includes(ownedItem.id) || usedForAttack === ownedItem.id) {
                    ...
                    selectedActor.updateEmbeddedDocuments('Item', [{
                      _id: i.id,
                      'system.consumables': consumablesList,
                      'system.useConsumableForAttack': usedForAttack
                    }]);
                  }
                }
              });
   ...
   */
  async _onPerformAttack(event, showThrowDiag) {
  ...
  updateWithItemSpecificValues(itemData, type, subtype = "otherInternal") {
    switch (type) {
      case "skills":
      ...
      case "weapon":
        if (game.settings.get('twodsix', 'hideUntrainedSkills')) {
          itemData.system.skill = this.actor.getUntrainedSkill().id;
        }
        if (!itemData?.img) {
  ...
  async _onItemCreate(event) {
    ...
    this.updateWithItemSpecificValues(itemData, type, header.dataset.subtype);
  ...
  async processDroppedItem(event, itemData) {
    ...
    return await this.actor.handleDroppedItem(itemData);
  }

  static _prepareItemContainers(actor, sheetData) {
    ...
    items.forEach((item) => {
      ...
      if (item.type === "component") {
        if (component[item.system.subtype] === undefined) {
          component[item.system.subtype] = [];
          summaryStatus[item.system.subtype] = {
            status: item.system.status,
            uuid: item.uuid
          };
        }
        component[item.system.subtype].push(item);
        if (statusOrder[summaryStatus[item.system.subtype].status] < statusOrder[item.system.status]) {
          summaryStatus[item.system.subtype] = {
            status: item.system.status,
            uuid: item.uuid
          };
  ...
  async _onRollInitiative(event) {
    ...
    if (this.token?.combatant?.id) {
      //@ts-ignore
      game.combat?.rollInitiative(this.token.combatant.id, {
        formula: dialogData.rollFormula,
        messageOptions: {rollMode: dialogData.rollMode}
      });
    } else {
      this.actor.rollInitiative({
        createCombatants: true,
        rerollInitiative: false,
        initiativeOptions: {formula: dialogData.rollFormula, messageOptions: {rollMode: dialogData.rollMode}}
      });
    }
  }
   ...
   * @private
   */
  async _onSkillRoll(event, showThrowDiag) {
    const item = this.getItem(event);
   ...
   * @private
   */
  async _onRollChar(event, showThrowDiag) {
  ...
  async _modifyEffect(event) {
    ...
    if (action === "delete") {
      ...
      const selectedEffect = await fromUuid(event.currentTarget["dataset"].uuid);
      if (selectedEffect) {
        await this.actor.updateEmbeddedDocuments("ActiveEffect", [{
          _id: selectedEffect.id,
          disabled: !selectedEffect.disabled
        }]);
        this.render(false);
      }
