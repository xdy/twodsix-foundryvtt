...
export class TwodsixActorSheet extends AbstractTwodsixActorSheet {
  ...
  async _onRefillConsumable(event) {
    const item = this.getConsumableItem(event);
    try {
      await item.refill();
    } catch (err) {
   ...
   * @private
   */
  async _onAutoAddConsumable(event) {
   ...
   * @private
   */
  async _onToggleConsumable(event) {
    const parentId = $(event.currentTarget).data("parentId");
    const consumableId = $(event.currentTarget).data("consumableId");
    const parentItem = await this.actor.items.get(parentId);
    const consumable = await this.actor.items.get(consumableId);
