      ...
      render: (_ev, html) => {
        ...
        html.element.querySelectorAll('.trade-cargo-row').forEach(row => {
          row.addEventListener('dragstart', (ev) => {
            const tradeData = (ev.currentTarget).getAttribute('data-trade');
            if (tradeData && ev.dataTransfer) {
              try {
                ev.dataTransfer.setData('text/plain', JSON.stringify({
                  type: 'trade-cargo',
                  row: JSON.parse(tradeData)
                }));
 ...
 * @returns {object}
 */
function buildWorldFeaturesContext() {
  const features = {};
  const iconMap = {
 ...
 * @returns {object}
 */
function buildStatLookup(options, fields) {
 ...
 * @param {object} input Object with stats or a world actor
 * @returns {string} UWP string (e.g. A123456-7A)
 */
export function generateUWP(input) {
  let stats;
  let features = [];
  if (input.system) {
    // Assume actor object
 ...
 * Accepts either a stats object or a world actor (with system property).
 * @param {object} input Object with stats or a world actor
 * @returns {{codes: Array<{code: string, tooltip: string}>, fillColor: string}}
 */
export function generateTradeCodes(input) {
  let stats;
  if (input.system) {
    stats = {
      starport: input.system.starport,
 ...
 * @param {string} code Trade code string (e.g. "Ag")
 * @returns {string} Localization string
 */
export function tradeCodeTooltip(code) {
