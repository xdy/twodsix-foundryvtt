...
import {deletePDFReference, getDataFromDropEvent, getItemDataFromDropData, openPDFReference} from "../utils/sheetUtils";
...
export class TwodsixItemSheet extends AbstractTwodsixItemSheet {
  ...
  async _changeType(event) {
    ...
    await this.item.toggleActiveEffectStatus(disableState);
  }
    ...
    if (this.actor?.type === "ship" || this.actor?.type === "vehicle") {
      ...
      if (game.settings.get('twodsix', 'useItemActiveEffects')) {
        const effects = [new ActiveEffect({
          ...
          transfer: true,
          disabled: this.item.system.equipped !== undefined && this.item.system.equipped !== "equipped" && !["augment", "trait"].includes(this.item.type),
          _id: newId,
  ...
  _onEditConsumable(event) {
    this.getConsumable(event)?.sheet?.render(true);
  }

  async _onDeleteConsumable(event) {
    const consumable = this.getConsumable(event);
    if (!consumable) {
      this.item.removeConsumable(""); //TODO Should have await?
    } else {
      const body = game.i18n.localize("TWODSIX.Items.Consumable.RemoveConsumableFrom").replace("_CONSUMABLE_NAME_", `"<strong>${consumable.name}</strong>"`).replace("_ITEM_NAME_", this.item.name);
      await Dialog.confirm({
        ...
        yes: async () => {
          if (consumable && consumable.id) {
            await this.item.removeConsumable(consumable.id); //TODO Should have await?
            this.render();
          }
        },
        no: () => {
      ...
      buttons: {
        ok: {
          ...
          callback: async (buttonHtml) => {
            const max = parseInt(buttonHtml.find('.consumable-max').val(), 10) || 0;
            let equippedState = "";
            ...
            const newConsumableData = {
              ...
              system: {
                ...
                quantity: parseInt(buttonHtml.find('.consumable-quantity').val(), 10) || 0,
                currentCount: max,
                max: max,
 ...
 * @export
 */
export function getDamageTypes(isWeapon) {
  const returnObject = {};
  if (game.settings.get('twodsix', 'damageTypeOptions') !== "") {
    let protectionTypeLabels = game.settings.get('twodsix', 'damageTypeOptions').split(',');
