...
Hooks.on("updateItem", async (item, update, _options, userId) => {
  if (game.user?.id === userId) {
    const owningActor = item.actor;
    ...
    if (game.settings.get('twodsix', 'useWoundedStatusIndicators') && owningActor) {
      if (checkForDamageStat(update, owningActor.type) && ["traveller", "animal", "robot"].includes(owningActor.type)) {
        await applyWoundedEffect(item.actor);
      }
    }
  }
});
...
Hooks.on("createItem", async (item, _options, userId) => {
  if (game.settings.get('twodsix', 'useEncumbranceStatusIndicators') && game.user?.id === userId) {
    ...
    if ((item?.actor?.type === 'traveller') && ["weapon", "armor", "equipment", "tool", "junk", "consumable"].includes(item.type)) {
      applyEncumberedEffect(item.actor).then();
    }
  }
});

function checkForWounds(systemUpdates, actorType) {
...
async function setConditionState(effectLabel, targetActor, state) {
  ...
  if (targetActor.isToken) {
    targetToken = targetActor.token.object;
  } else {
    targetToken = canvas.tokens?.ownedTokens.find(t => t.actor?.id === targetActor.id);
  ...
  if (isAlreadySet.length > 1 && targetToken) {
    ...
    for (let i = 1; i < isAlreadySet.length; i++) {
      await targetToken.toggleEffect(targetEffect, {active: false});
    }
  }
  if ((isAlreadySet.length > 0) !== state) {
    if (targetToken && targetEffect) {
      if (effectLabel === "Dead") {
        await targetToken.toggleEffect(targetEffect, {active: state, overlay: true});
        // Set defeated if in combat
        const fighters = game.combats?.active?.combatants;
        const combatant = fighters?.find((f) => f.tokenId === targetToken.id);
        if (combatant !== undefined) {
          await combatant.update({defeated: state});
        }
      } else {
        await targetToken.toggleEffect(targetEffect, {active: state});
      }
    }
  }
}

async function setWoundedState(effectLabel, targetActor, state, tint) {
  ...
  if (state) {
    ...
    if (!currentEffectId) {
      ...
      if (currentEfffect.tint !== tint) {
        await targetActor.updateEmbeddedDocuments('ActiveEffect', [{
          _id: currentEffectId,
          tint: tint,
          changes: [changeData]
        }]);
...
export function getIconTint(selectedActor) {
...
export function getHitsTint(selectedTraveller) {
  let returnVal = '';
...
export function getCDWoundTint(selectedTraveller) {
  let returnVal = '';
  if (selectedTraveller.characteristics.lifeblood.current <= 0) {
    returnVal = DAMAGECOLORS.deadTint;
...
export function getCELWoundTint(selectedTraveller) {
  let returnVal = '';
...
export function getCEWoundTint(selectedTraveller) {
  let returnVal = '';
  const testArray = [selectedTraveller.characteristics.strength, selectedTraveller.characteristics.dexterity, selectedTraveller.characteristics.endurance];
  const maxNonZero = testArray.filter(chr => chr.value !== 0).length;
  const currentZero = testArray.filter(chr => chr.current <= 0 && chr.value !== 0).length;
...
export function isUnconsciousCE(selectedTraveller) {
  const testArray = [selectedTraveller.characteristics.strength, selectedTraveller.characteristics.dexterity, selectedTraveller.characteristics.endurance];
  return (testArray.filter(chr => chr.current <= 0 && chr.value !== 0).length === 2);
}

export function getCEAWoundTint(selectedTraveller) {
  let returnVal = '';
