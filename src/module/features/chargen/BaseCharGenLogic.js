...
export class BaseCharGenLogic {
  ...
  async loadCareerData(ruleset, { careersPackName = null, careersFallbackPackName = null } = {}) {
    // Backward-compatible override: if subclass provides _getCareerPackOptions, use it.
    // Otherwise resolve via CharGenPackResolver.
    let primaryPack;
    if (careersPackName != null) {
      primaryPack = game.packs.get(careersPackName) || (careersFallbackPackName ? game.packs.get(careersFallbackPackName) : null);
    } else {
      primaryPack = resolvePack(ruleset, PackType.CAREERS);
    }
    if (!primaryPack) {
      throw new Error(`Failed to load career data: no pack found for ruleset "${ruleset}".`);
    }
