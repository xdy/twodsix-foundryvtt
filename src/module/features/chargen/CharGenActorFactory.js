...
import { loadDocumentsFromPackChain, resolveItemPackChain, resolveTraitPackChain } from './CharGenPackResolver.js';
...
export async function createCharacterActor(state, charName) {
  ...
  const itemIndexes = await _loadAllItemIndexes(state.ruleset);
...
async function _loadAllItemIndexes(ruleset) {
  const itemChain = resolveItemPackChain(ruleset);
  const itemDocs = await loadDocumentsFromPackChain(itemChain);

  const traitChain = resolveTraitPackChain(ruleset);
  const traitDocs = await loadDocumentsFromPackChain(traitChain);
...
function _buildAllItemIndexes(itemDocs, traitDocs) {
  ...
  for (const doc of itemDocs) {
    ...
    if (doc.type === 'skills' && !skillByName.has(doc.name)) {
      skillByName.set(doc.name, doc);
    }
  }
  for (const doc of traitDocs) {
    if (doc?.type === 'trait' && !traitByName.has(doc.name)) {
...
async function _attachSkills(actor, state, skillByName, ruleset) {
  ...
  if (missingSkills.length) {
    console.warn(
      `twodsix | CharGenActorFactory: ${missingSkills.length} skill(s) missing from pack for ruleset "${ruleset}": ${JSON.stringify(missingSkills)}`,
    );
  }
}
...
async function _attachWeapons(actor, state, itemById, ruleset) {
  ...
  if (missingWeaponIds.length) {
    console.warn(
      `twodsix | CharGenActorFactory: ${missingWeaponIds.length} selected weapon id(s) missing from pack for ruleset "${ruleset}": ${JSON.stringify(missingWeaponIds)}`,
    );
  }
}
...
async function _attachTraits(actor, state, traitByName, ruleset) {
  ...
  if (missingTraitNames.length) {
    console.warn(
      `twodsix | CharGenActorFactory: ${missingTraitNames.length} trait(s) missing from trait pack for ruleset "${ruleset}": ${JSON.stringify(missingTraitNames)}`,
    );
  }
}
...
async function _attachSpecies(actor, state, ruleset) {
  ...
  if (missingSpecies.length) {
    console.warn(
      `twodsix | CharGenActorFactory: species missing from pack for ruleset "${ruleset}": ${JSON.stringify(missingSpecies)}`,
    );
  }
}
...
async function _buildSpeciesItem(state) {
  ...
  const overlay = state?.chargenOverlay?.[overlayKey];
  const speciesRef = overlay?.species;
  ...
  try {
    ...
    if (doc && doc.type === 'species') {
      const data = _sanitizeEmbeddedItemData(doc.toObject());
      _patchSpeciesAeDeltas(data, overlay);
      speciesToAdd.push(data);
    } else {
...
function _patchSpeciesAeDeltas(speciesData, overlay) {
  const cd = overlay?.centaurDice;
  if (!cd || typeof cd !== 'object') {
    return;
  }
  const effects = speciesData.effects;
  if (!Array.isArray(effects) || !effects.length) {
    return;
  }
  const ae = effects.find(e => e.name && e.name.toLowerCase().includes('characteristics'));
  if (!ae) {
    return;
  }
  const changes = ae.system?.changes;
  if (!Array.isArray(changes)) {
    return;
  }
  if (typeof cd.str === 'number' && cd.str !== 0) {
    changes.push({ key: 'system.characteristics.strength.value', type: 'add', value: String(cd.str), priority: 20 });
  }
  if (typeof cd.end === 'number' && cd.end !== 0) {
    changes.push({ key: 'system.characteristics.endurance.value', type: 'add', value: String(cd.end), priority: 20 });
  }
}
