...
export const CDEE_PRISON_EVENTS = [
  ...
  // Structured row: resolved via CDEECharGenLogic.applyEventTags -> eventUsesStructuredResolve -> _resolveEvent (not flat effects[]).
  {
    roll: 5,
    description:
      'Smuggling. Optionally stake one benefit roll; Streetwise 8+ wins an extra roll or loses your stake.',
    checks: [{ skill: 'Streetwise', target: 8 }],
    benefitGamble: {
      kind: 'optional_single',
      promptKey: 'TWODSIX.CharGen.CDEE.BenefitGamble.OptionalRiskPromptPrison',
    },
    onSuccess: [],
    onFail: [],
  },
  ...
  { roll: 8, description: 'Prison Riot! [CHECK:Melee Combat:8] or [CHECK:Stealth:8] or [CHECK:Liaison:8] to avoid [INJURY]. [IMPROVE_CHECK_SKILL]' },
...
function _withStructuredEffects(table) {
  return table.map(entry => {
    const hasStructuredChargen =
      (entry.checks?.length ?? 0) > 0
      || (entry.branchChoices?.length ?? 0) > 0
      || (entry.benefitGamble?.kind != null)
      || (entry.onSuccess?.length ?? 0) > 0
      || (entry.onFail?.length ?? 0) > 0
      || (entry.always?.length ?? 0) > 0;
    if (hasStructuredChargen) {
      return { ...entry };
    }
    return {
      ...entry,
