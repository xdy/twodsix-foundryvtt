...
export async function cuStepDesignCharacteristicsString(_logic, app) {
  const selection = await app._choose(
    game.i18n.localize('TWODSIX.CharGen.Steps.CUChooseCharacteristicString'),
    [
...
export async function cuStepDesignAge(_logic, app) {
  ...
  const choice = await app._choose(
    game.i18n.localize('TWODSIX.CharGen.Steps.CUAgeDesignDecideOrRoll'),
    [
  ...
  if (choice === 'roll') {
    ...
    bracket = await app._choose(
      game.i18n.localize('TWODSIX.CharGen.Steps.CUAgeDesignPickBracket'),
      [
...
export async function cuResolveVehicleSpecialization(_logic, app, skillName) {
  ...
  return await app._choose(
    game.i18n.localize('TWODSIX.CharGen.Steps.CUVehicleSpecialization'),
    [
...
export async function cuStepDesignCareerSkills(logic, app) {
  const state = app.charState;
  const careerKeys = Object.keys(CU_DESIGN_CAREERS);
...
export async function cuApplyDesignBonusSkill(logic, app) {
  const state = app.charState;
  const tableChoice = await app._choose(
    game.i18n.localize('TWODSIX.CharGen.Steps.CUBonusSkillDesignTable'),
    [
