...
import { formatAutoHandledSuffix } from '../../EventReport.js';
    ...
    while (true) {
      ...
      if (event) {
        ...
        if (event.description.includes('[AUTO_PROMO_OR_PRISON]')) {
          ...
          const headline = eventReport.allAutoHandled ? `${cleanDesc}${formatAutoHandledSuffix(eventReport)}` : cleanDesc;
      ...
      if (promoEvent) {
        ...
        for (const tag of remainingPromoTags) {
          ...
          promoAutoHandledItems.push(...(er._autoHandledItems || []));
        }

        const promoHeadline = promoAllAuto ? `${cleanDesc}${formatAutoHandledSuffix(promoAutoHandledItems)}` : cleanDesc;
