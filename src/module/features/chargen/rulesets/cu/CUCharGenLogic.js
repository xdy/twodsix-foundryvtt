...
import { runCUCareerTerms } from './cuCareerTerms.js';
import {
  ...
  cuStepDesignRank,
} from './cuDesignSteps.js';
import { cuBenefitRoll, cuMusterOut } from './cuMusterBenefits.js';
import {
...
export class CUCharGenLogic extends BaseCharGenLogic {
  ...
  _getCareerPackOptions(ruleset) {
    return { careersPackName: 'twodsix.cepheus-universal-items' };
  }
  ...
  async run(app) {
    ...
    const mode = await app._choose(
      game.i18n.localize('TWODSIX.CharGen.Steps.CUCreationMethod'),
      [
  ...
  async runRandomMode(app) {
    ...
    const rollOrPick = await app._choose(
      game.i18n.localize('TWODSIX.CharGen.Steps.CUSkillTableMethod'),
      [
  ...
  async _applyTableBonusSkill(app, tableA, tableB) {
    ...
    const table = await app._choose(
      game.i18n.localize('TWODSIX.CharGen.Steps.CUBonusSkillPickTable'),
      [
