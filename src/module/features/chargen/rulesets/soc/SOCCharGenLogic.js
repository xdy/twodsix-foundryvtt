...
export class SOCCharGenLogic extends BaseCharGenLogic {
  ...
  _getCareerPackOptions() {
    return {
      careersPackName: 'twodsix.sword-of-cepheus-items',
    };
  }
  ...
  _assignRulesetConstants(careers, careerNames, ruleset) {
    ...
    if (entry?.packs?.itemPackId) {
      this.traitPackNames.push(entry.packs.itemPackId);
    } else {
      this.traitPackNames.push(`twodsix.${ruleset.toLowerCase()}-srd-items`);
    }
    if (entry?.packs?.itemPackFallbackId) {
      this.traitPackNames.push(entry.packs.itemPackFallbackId);
    }
    // Ensure SOC items are checked if not already there
    if (!this.traitPackNames.includes('twodsix.sword-of-cepheus-items')) {
      this.traitPackNames.push('twodsix.sword-of-cepheus-items');
    }
  }
  ...
  async stepSOCSpeciesPath(app) {
    ...
    if (mode === 'revenant') {
      ...
      const rev = revChoice ? {
        ...
        deltas: revChoice.deltas ?? { str: 0, dex: 0, end: 0, int: 0, edu: 0, soc: 0 },
    ...
    const profileData = {
      id: speciesId,
      displayName: name,
      deltas: speciesChoice?.deltas ?? { str: 0, dex: 0, end: 0, int: 0, edu: 0, soc: 0 },
  ...
  async applySocSpeciesCharacteristicAdjustments(app) {
    ...
    if (profile.centaurStrEndDice) {
      ...
      adjustChar(state, 'str', dStr);
      adjustChar(state, 'end', dEnd);
