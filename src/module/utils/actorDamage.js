...
import { calcModFor, getDamageTypes } from './sheetUtils';
...
export class Stats {
   ...
   * @param {number} armorPiercingValue
   * @param {string} damageType
   * @param {string} damageLabel
   * @param {number} [parryArmor=0]
   * @param {boolean} [canOnlyBeBlocked=false]
   * @param {object[]} [dice=[]]
   */
  constructor(actor, damageValue, armorPiercingValue, damageType, damageLabel, parryArmor = 0, canOnlyBeBlocked = false, dice = []) {
    ...
    if (actor.type !== "ship") {
      this.primaryArmor = this.damageType === 'psionic' ? 0 : (actor.system).primaryArmor.value; //primary armor does not stop psionic damage
 ...
 * @returns {Promise<void>}
 */
export async function renderDamageDialog(damageData) {
  const {
    damageId,
    damageValue,
    armorPiercingValue,
 ...
 * @returns {void}
 */
export function destroyDamageDialog(damageId) {
 ...
 * @returns {string[]}
 */
export function getDamageCharacteristics(actorType) {
 ...
 */
export async function getParryValue(actor, canOnlyBeBlocked) {
  let returnValue = 0;
    ...
    if (meleeSkill) {
      ...
      if (weaponsList?.length > 0) {
        ...
        if (weapon.system.parryAV > 0) {
          ...
          const settings = await TwodsixRollSettings.create(false, tmpSettings, meleeSkill, undefined, actor);
 ...
 * @returns {number}
 */
export function stackArmorValues(base, add) {
  const isFrac = (v) => v > 0 && v < 1;
