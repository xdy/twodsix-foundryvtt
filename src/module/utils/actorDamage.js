 ...
 */
export class Stats {
  constructor(actor, damageValue, armorPiercingValue, damageType) {
    this.edited = false;
    ...
    if (actor.type !== "ship") {
      this.primaryArmor = actor.system.primaryArmor.value;
  ...
  totalDamage() {
    ...
    if (Roll.validate(damageFormula)) {
      const totalDamage = new Roll(damageFormula, rollData).evaluate({async: false}).total;
...
class DamageDialogHandler {
  ...
  getNumericValueFromEvent(event, upper) {
...
export async function renderDamageDialog(damageData) {
  ...
  if (damageData.actorId) {
    ...
    actor = (canvas.tokens?.placeables?.find((t) => t.id === damageData.tokenId) || null)?.actor || null;
  }
  ...
  new Dialog({
    ...
    render: (html) => damageDialogHandler.setHtml(html),
