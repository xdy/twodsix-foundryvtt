 ...
 * @returns {number}
 */
export function calcModFor(characteristic) {
  let modifier = 0;
 ...
 * @returns {object}
 */
export function getDataFromDropEvent(ev) {
  try {
 ...
 */
export async function getDocFromDropData(dropData) {
  let doc;
...
/**
 * @param {string} dropString
 * @returns {{type: string, href: string, label: string}}
 */
export function getHTMLLink(dropString) {
 ...
 * @returns {void}
 */
export function openPDFLink(ev, target) {
 ...
 */
export async function deletePDFLink(ev, target) {
 ...
 * @returns {Promise<void>}
 */
export async function addPDFLink/*ev: Event, target:HTMLElement*/() {
  const newRefArray = foundry.utils.duplicate(this.document.system.docReference);
  newRefArray.push("");
  await this.document.update({"system.docReference": newRefArray});
}
 ...
 * @returns {Promise<void>}
 */
export async function changeReference(ev) {
  ev.preventDefault();
 ...
 * @returns {Promise<void>}
 */
export async function deleteReference(ev) {
  ev.preventDefault();
  if (!this.document.isOwner) {
    ui.notifications.warn("TWODSIX.Warnings.LackPermissionToEdit", {localize: true});
    return;
  ...
  if (this.document.system.pdfReference.href !== "") {
    await this.document.update({
      "system.pdfReference.type": "",
      "system.pdfReference.href": "",
 ...
 * @returns {boolean}
 */
export function isDisplayableSkill(skill) {
 ...
 * @export
 */
export function getRollTypeSelectObject() {
  const returnObj = {};
 ...
 * @export
 */
export function getDifficultiesSelectObject(
  difficultyList = TWODSIX.DIFFICULTIES[game.settings.get('twodsix', 'difficultyListUsed')]
) {
  const returnObj = {};
 ...
 * @export
 */
export function getConsumableOptions(item) {
  const returnObj = {"": "---"};
 ...
 * @export
 */
export function getRangeTypes(labelType = 'short') {
