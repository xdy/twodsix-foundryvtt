...
export function getDataFromDropEvent(event) {
  try {
...
export async function getItemDataFromDropData(dropData) {
  let item;
...
export function openPDFReference(sourceString) {
  if (sourceString) {
...
export async function deletePDFReference(event) {
...
export async function confirmRollFormula(initFormula, title) {
  const returnText = await new Promise((resolve) => {
    new Dialog({
      title: title,
      content: `<label>Formula</label><input type="text" name="outputFormula" id="outputFormula" value="` + initFormula + `"></input>`,
      buttons: {
        Roll: {
