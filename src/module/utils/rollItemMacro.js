...
export function shouldShowCELAutoFireDialog(weapon) {
  const rateOfFire = weapon.system.rateOfFire;
  return ((game.settings.get('twodsix', 'autofireRulesUsed') === TWODSIX.RULESETS.CEL.key) &&
    (Number(rateOfFire) > 1));
}

export function shouldShowCEAutoFireDialog(weapon) {
  const modes = (weapon.system.rateOfFire ?? "").split(/[-/]/);
  return ((game.settings.get('twodsix', 'autofireRulesUsed') === TWODSIX.RULESETS.CE.key) &&
    (modes.length > 1));
}

export async function promptForCELROF() {
...
export async function promptAndAttackForCE(modes, item) {
  const buttons = {};
...
export async function resolveUnknownAutoMode(item) {
  let attackType = "";
