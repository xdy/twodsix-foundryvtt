 ...
 */
async function processDocuments(documents, fn, options = {}) {
  if (options.batch) {
    const updates = [];
 ...
 * @param {string | undefined} packCollection
 * @returns {Promise<void>}
 */
async function applyBatchedUpdates(updates, DocumentClass, packCollection) {
 ...
 *
 * @param {Function} fn - A function (sync or async) to apply to each TwodsixActor. Can either:
 *             - Return void and call actor.update() itself (old behavior)
 *             - Return an update object to be batched (new batching behavior)
 * @param {object} options - Optional configuration
 * @param {boolean} options.batch - If true, collects updates and applies them in batches. Default: false
 * @returns {Promise<void>} A promise that resolves when all actors and packs have been processed.
 */
export async function applyToAllActors(fn, options = {}) {
 ...
 *
 * @param {Function} fn - A function (sync or async) to apply to each TwodsixItem. Can either:
 *             - Return void and call item.update() itself (old behavior)
 *             - Return an update object to be batched (new batching behavior)
 * @param {object} options - Optional configuration
 * @param {boolean} options.batch - If true, collects updates and applies them in batches. Default: false
 * @returns {Promise<void>} A promise that resolves when all items and packs have been processed.
 */
export async function applyToAllItems(fn, options = {}) {
 ...
 *
 * @param {Function} fn - A function (sync or async) to apply to each document in the pack. Can either:
 *             - Return void and call doc.update() itself (old behavior)
 *             - Return an update object to be batched (new batching behavior)
 * @param {CompendiumCollection[]} packs - An array of compendium collections to process.
 * @param {object} options - Optional configuration
 * @param {boolean} options.batch - If true, collects updates and applies them in batches. Default: false
 * @returns {Promise<void>} A promise that resolves when all packs have been processed.
 */
async function applyToAllPacks(fn, packs, options = {}) {
