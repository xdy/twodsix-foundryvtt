...
export class TwodsixDiceRoll {
  ...
  public async sendToChat(difficultyList: object):Promise<void> {
    ...
    let flavorText = this.settings.extraFlavor ? this.settings.extraFlavor + `<br>`: ``;
    let flavorTable = `<table><tr><th>${game.i18n.localize("TWODSIX.Chat.Roll.Modifier")}</th><th>${game.i18n.localize("TWODSIX.Chat.Roll.Description")}</th><th>${game.i18n.localize("TWODSIX.Chat.Roll.DM")}</th></tr>`;

    //Difficulty Text
    flavorText += `${rollingString}: ${difficulty}`;
    flavorTable += `<tr><td>${game.i18n.localize("TWODSIX.Chat.Roll.Difficulty")}</td><td>${difficulty}</td>`;
    if (game.settings.get('twodsix', 'difficultiesAsTargetNumber')) {
      flavorText += `(${this.settings.difficulty.target}+)`;
      flavorTable += `<td>${this.settings.difficulty.target}+</td></tr>`;
    } else {
      const difficultyMod = TwodsixDiceRoll.addSign(this.settings.difficulty.mod);
      flavorText += `(${difficultyMod})`;
      flavorTable += `<td>${difficultyMod}</td></tr>`;
    }
    ...
    if (this.settings.rollType != TWODSIX.ROLLTYPES.Normal.key) {
      ...
      flavorTable += `<tr><td>${game.i18n.localize("TWODSIX.Chat.Roll.Type")}</td><td>${rollType}</td><td></td></tr>`;
    }

    //Skill Level
    if (this.settings.skillRoll) {
      const skillValue = TwodsixDiceRoll.addSign(this.settings.rollModifiers.skill);
      flavorText += ` ${usingString} ${this.settings.skillName}(${skillValue}) ${game.i18n.localize("TWODSIX.itemTypes.skill")}`;
      flavorTable += `<tr><td>${game.i18n.localize("TWODSIX.Chat.Roll.SkillModifier")}</td><td>${this.settings.skillName}</td><td>${skillValue}</td></tr>`;
    }
    ...
    if (this.settings.rollModifiers.characteristic !== 'NONE' && this.actor) { //TODO Maybe this should become a 'characteristic'? Would mean characteristic could be typed rather than a string...
      ...
      flavorText += this.settings.skillRoll ? ` & ${charShortName}(${characteristicValue}) ${characteristicLabel}` : ` ${usingString} ${charShortName}(${characteristicValue}) ${characteristicLabel}`;
      flavorTable += `<tr><td>${game.i18n.localize("TWODSIX.Chat.Roll.Characteristic")}</td><td>${charShortName}</td><td>${characteristicValue}</td></tr>`;
    }

    //Item & Attack Modifiers
    if (this.settings.itemRoll) {
      const itemValue = TwodsixDiceRoll.addSign(this.settings.rollModifiers.item);
      flavorText += this.settings.skillRoll ? ` & ${this.settings.itemName}(${itemValue})` : ` ${usingString} ${this.settings.itemName}(${itemValue})`;
      flavorTable += `<tr><td>${game.i18n.localize("TWODSIX.Chat.Roll.ItemModifier")}</td><td>${this.settings.itemName}</td><td>${itemValue}</td></tr>`;

      if (this.settings.rollModifiers.rof) {
        flavorText += ` + ${game.i18n.localize("TWODSIX.Rolls.ROF")} (${TwodsixDiceRoll.addSign(this.settings.rollModifiers.rof)})`;
        flavorTable += `<tr><td>${game.i18n.localize("TWODSIX.Chat.Roll.Attack")}</td><td>${game.i18n.localize("TWODSIX.Chat.Roll.ROF")}</td><td>${TwodsixDiceRoll.addSign(this.settings.rollModifiers.rof)}</td></tr>`;
      }
      if (this.settings.rollModifiers.dodgeParry && game.settings.get("twodsix", "useDodgeParry")) {
        flavorText += ` + ${game.i18n.localize("TWODSIX.Rolls.DodgeParry")} (${TwodsixDiceRoll.addSign(this.settings.rollModifiers.dodgeParry)})`;
        flavorTable += `<tr><td>${game.i18n.localize("TWODSIX.Chat.Roll.Attack")}</td><td>${game.i18n.localize("TWODSIX.Rolls.DodgeParry")}</td><td>${TwodsixDiceRoll.addSign(this.settings.rollModifiers.dodgeParry)}</td></tr>`;
      }
    }

    //Custom Modifier
    if (this.settings.rollModifiers.other !== 0) {
      flavorText += ` + ${game.i18n.localize("TWODSIX.Chat.Roll.Custom")}(${TwodsixDiceRoll.addSign(this.settings.rollModifiers.other)})`;
      flavorTable += `<tr><td>${game.i18n.localize("TWODSIX.Chat.Roll.Custom")}</td><td>&mdash;</td><td>${TwodsixDiceRoll.addSign(this.settings.rollModifiers.other)}</td></tr>`;
    }

    if (this.settings.rollModifiers.wounds !== 0) {
      flavorText += ` + ${game.i18n.localize("TWODSIX.Chat.Roll.Wounds")}(${this.settings.rollModifiers.wounds})`;
      flavorTable += `<tr><td>${game.i18n.localize("TWODSIX.Chat.Roll.Condition")}</td><td>${game.i18n.localize("TWODSIX.Chat.Roll.Wounds")}</td><td>${this.settings.rollModifiers.wounds}</td></tr>`;
    }
    if (this.settings.rollModifiers.encumbered !== 0) {
      flavorText += ` + ${game.i18n.localize("TWODSIX.Chat.Roll.Encumbered")}(${this.settings.rollModifiers.encumbered})`;
      flavorTable += `<tr><td>${game.i18n.localize("TWODSIX.Chat.Roll.Condition")}</td><td>${game.i18n.localize("TWODSIX.Chat.Roll.Encumbered")}</td><td>${this.settings.rollModifiers.encumbered}</td></tr>`;
    }
    if (this.settings.rollModifiers.custom !== 0) {
      flavorText += ` + ${game.i18n.localize("TWODSIX.Chat.Roll.Custom")}(${this.settings.rollModifiers.custom})`;
      flavorTable += `<tr><td>${game.i18n.localize("TWODSIX.Chat.Roll.Condition")}</td><td>${game.i18n.localize("TWODSIX.Chat.Roll.Custom")}</td><td>${this.settings.rollModifiers.custom}</td></tr>`;
    }
    ...
    flavorTable += `</table>`;
    const flavor = `<section class="dice-roll">`+ flavorText + `<section class="dice-tooltip">` + flavorTable + `</section></section>`;
