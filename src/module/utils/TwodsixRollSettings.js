...
import { addSign, getCharacteristicFromDisplayLabel, getKeyByValue, simplifySkillName } from './utils';
...
export class TwodsixRollSettings {
   ...
   * @param {TwodsixItem | null} [aSkill]
   * @param {TwodsixItem | null} [anItem]
   * @param {TwodsixActor | null} [sourceActor]
   */
   ...
   * @param {object} settings
   * @param {TwodsixItem | null} skill
   * @param {TwodsixItem | null} item
   * @param {TwodsixActor | null} sourceActor
  ...
  static async create(showThrowDialog, settings, skill, item, sourceActor) {
    ...
    if (showThrowDialog) {
      ...
      if (item && skill) {
        ...
        if (_getTranslatedCharacteristicList(skill.actor)[(twodsixRollSettings.rollModifiers.characteristic)] === undefined) {
...
/**
 * @param {Event} ev
 * @param {foundry.applications.api.ApplicationV2} htmlRend
 * @returns {void}
 */
...
/**
 * @param {TwodsixActor | null} actor
 * @returns {object}
 */
export function _getTranslatedCharacteristicList(actor) {
  const returnValue = {};
  if (actor) {
 ...
 * @returns {string}
 */
export function getCharacteristicLabelWithMod(actor, characteristic) {
 ...
 * @returns {object}
 */
export function _genUntranslatedCharacteristicList() {
  const returnValue = {};
 ...
 * @returns {object}
 */
export function getCharacteristicList(actor) {
  let returnValue = {};
  if (actor) {
 ...
 */
export function getCustomModifiers(selectedActor, characteristic, skill) {
 ...
 */
export function getInitialSettingsFromFormula(parseString, actor) {
