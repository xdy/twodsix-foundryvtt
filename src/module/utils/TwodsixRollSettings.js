...
export class TwodsixRollSettings {
  ...
  constructor(settings, aSkill, anItem, sourceActor) {
    ...
    let rollClass = "";
    const bonusDamage = settings?.bonusDamage ?? "";
    let woundsValue = 0;
    ...
    if (selectedActor) {
      ...
      const joat = (selectedActor.getUntrainedSkill().system)?.value ?? game.system.template?.Item?.skills?.value;
      if (joat > skillValue) {
        skillValue = joat;
...
export function getCharacteristicLabelWithMod(actor, characterisitc) {
...
export async function getCustomModifiers(selectedActor, characteristic, skill) {
