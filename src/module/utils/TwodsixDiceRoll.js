...
export class TwodsixDiceRoll {
  ...
  /**
   * @param {TwodsixRollSettings} rollSettings
   * @param {TwodsixActor} actor
   * @param {TwodsixItem | null} [skill=null]
   * @param {TwodsixItem | null} [item=null]
   */
  ...
  async sendToChat(difficultyList) {
    ...
    if (this.isSuccess() && !game.settings.get("twodsix", "automateDamageRollOnHit") && (this.item?.type === "weapon" || (this.item?.type === "component" && this.item?.system?.isArmament))) {
    ...
    } else if (this.rollSettings.skillRoll && this.item?.type !== "weapon" && !(this.item?.type === "component" && this.item?.system?.isArmament)) {
