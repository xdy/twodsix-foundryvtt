...
class DamageDialogHandler {
  ...
  public unRegisterListeners() {
    Object.entries(this.hooks).forEach(([name, hook]:[string,number]) => Hooks.off(name, hook));
    this.html.off('change', "**");
  }
}

export async function renderDamageDialog(damageData:Record<string,any>) {
  ...
  const html = await renderTemplate(template, {stats: damageDialogHandler.stats});
...
export function destroyDamageDialog(damageId:string) {
  Object.values(ui.windows).forEach(window => {
    if (window instanceof Dialog && window.id === damageId) {
      window.close()
    }
  });
}