...
import {_genUntranslatedCharacteristicList, _getTranslatedCharacteristicList} from "./utils/TwodsixRollSettings";
...
export default function registerHandlebarsHelpers() {
  ...
  Handlebars.registerHelper('twodsix_skillTotal', (actor, skillItem) => {
    ...
    if (adjValue === game.system.template.Item?.skills?.value && !game.settings.get("twodsix", "hideUntrainedSkills")) {
  ...
  Handlebars.registerHelper('twodsix_getTooltip', (actor, field) => {
    let returnValue = ``;
  ...
  Handlebars.registerHelper('getComponentWeight', (item) => {
    return getWeight(item.system, item.actor).toLocaleString(game.i18n.lang, {
  ...
  Handlebars.registerHelper('getComponentPower', (item) => {
    ...
    const retValue = getPower(anComponent);
  ...
  Handlebars.registerHelper('twodsix_getEffectSource', (effect) => {
    if (["Wounded", "Encumbered", "Dead"].includes(effect.label)) {
      ...
      const attachedItem = fromUuid(effect.origin);
      if (attachedItem) {
