...
export default class TwodsixItem extends Item {
  ...
  prepareConsumable(gear = this.system) {
    if (gear.consumables !== undefined && gear.consumables.length > 0 && this.actor != null) {
      ...
      const allConsumables = gear.consumables.map((consumableId) => {
  ...
  //////// WEAPON ////////
  async performAttack(attackType, showThrowDialog, rateOfFireCE = null, showInChat = true, weapon = this.system) {
    if (this.type !== "weapon") {
      return;
    }
    if (!weapon.skill) {
    ...
    if (game.settings.get("twodsix", "useDodgeParry")) {
      ...
      if (game.user?.targets) {
        const selectedTarget = Array.from(game.user.targets)[0]?.actor;
        const targetMatchingSkill = selectedTarget?.itemTypes.skills.find(sk => sk.name === skillName);
        const dodgeParryModifier = targetMatchingSkill?.system.value || 0;
    ...
    if (weapon.useConsumableForAttack) {
      const magazine = this.actor?.items.get(weapon.useConsumableForAttack);
      if (magazine) {
        try {
    ...
    for (let i = 0; i < numberOfAttacks; i++) {
      ...
      if (game.settings.get("twodsix", "automateDamageRollOnHit") && roll && roll.isSuccess()) {
        ...
        if (game.user?.targets.size === 1 && damagePayload) {
          game.user?.targets.values().next().value.actor.handleDamageData(damagePayload, !game.settings.get('twodsix', 'invertSkillRollShiftClick'));
  ...
  async skillRoll(showThrowDialog, tmpSettings, showInChat = true) {
    ...
    if (this.type == "skills") {
      ...
      if (skill === undefined) {
        skill = this.actor.getUntrainedSkill();
      }
    ...
    if (usesConsumable.useConsumableForAttack && item && item.type != "weapon") {
      const magazine = this.actor?.items.get(usesConsumable.useConsumableForAttack); //this shoould always be this.actor as components on ship, not working actor
      if (magazine) {
        try {
  ...
  async rollDamage(rollMode, bonusDamage = "", showInChat = true, confirmFormula = false) {
  ...
  getConsumableBonus(type) {
    ...
    if (this.system.useConsumableForAttack && this.actor) {
      ...
      if (magazine?.type === "consumable" && magazine?.system[type]) {
        returnValue += magazine.system[type];
      }
    }
    return returnValue;
  }
   ...
   */
  async toggleActiveEffectStatus(newSuspendedState) {
 ...
 * @private
 */
export async function onRollDamage(event) {
  ...
  await item.rollDamage(game.settings.get('core', 'rollMode'), bonusDamageFormula, true, showFormulaDialog);
}
