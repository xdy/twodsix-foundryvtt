...
/** @typedef {import("@client/applications/ux/context-menu.mjs").ContextMenuEntry} ContextMenuEntry */
 ...
 */
export async function onChatCardAction(event, target) {
  ...
  if (effect > 0) {
    return Promise.all(canvas.tokens.controlled.map(t => {
      ...
      if (["traveller", "robot", "animal", "ship"].includes(targetActor.type)) {
        ...
        if (damage > 0) {
          const damagePayload = transfer?.payload ?? {
            armorPiercingValue: 0,
            damageLabel: "",
            damageType: "",
            dice: message.rolls[0]?.dice[0]?.results
          };
...
async function makeSecondaryRoll(message, type, showDialog) {
  ...
  const settings = await TwodsixRollSettings.create(showDialog, tempSettings, selectedSkill, undefined, secondActor);
  ...
  if (roll && type === "opposed") {
    ...
    if (winnerName === "") {
      ChatMessage.create({
        content: game.i18n.localize("TWODSIX.Chat.Roll.TiedRoll"),
        speaker: ChatMessage.getSpeaker({actor: secondActor})
      });
    } else {
      ChatMessage.create({
        content: `${winnerName} ${game.i18n.localize("TWODSIX.Chat.Roll.WinsRoll")}`,
        speaker: ChatMessage.getSpeaker({actor: secondActor})
      });
    }
  }
