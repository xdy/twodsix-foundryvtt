...
export default class TwodsixActor extends Actor {
  ...
  async _prepareTravellerData() {
    ...
    const actorSkills = this.itemTypes.skills.map((skill) => [simplifySkillName(skill.name ?? ""), skill.system.value]);
    const handler = {
      has: (target, property) => {
      ...
      get: (target, property) => {
        if (property[property.length - 1] === "_") {
          ...
          return property in target ? target[property] : this.getUntrainedSkill().system.value;
        }
      }
    };
  ...
  _prepareShipData() {
    ...
    function calculateComponentCost(anComponent, weightForItem, shipActor) {
    ...
    function updateShipData(shipActor) {
      ...
      shipActor.system.shipValue = calcShipStats.cost.total.toLocaleString(game.i18n.lang, {
  ...
  _onUpdateEmbeddedDocuments(embeddedName, documents, result, options, userId) {
    ...
    if (game.user.id === userId) {
      if (embeddedName === "ActiveEffect" && !result[0].flags && !options.dontSync && game.settings.get('twodsix', 'useItemActiveEffects')) {
        documents.forEach(async (element, i) => {
  ...
  async damageActor(damageValue, armorPiercingValue, damageType, showDamageDialog = true) {
    if (showDamageDialog) {
      const damageData = {
   ...
   * @private
   */
  async _addDroppedSkills(skillData) {
    ...
    const addedSkill = (await this.sheet._onDropItemCreate(duplicate(skillData)))[0];
    ...
    if (addedSkill.system.value < 0 || !addedSkill.system.value) {
      if (!game.settings.get('twodsix', 'hideUntrainedSkills')) {
        const skills = game.system.template.Item?.skills;
   ...
   * @private
   */
  async _addDroppedEquipment(itemData) {
      ...
      if (itemData.system.quantity > 1) {
        ...
        if (numberToMove >= itemData.system.quantity) {
          ...
          await sourceActor.updateEmbeddedDocuments("Item", [{
            _id: itemData.id,
            'system.quantity': (itemData.system.quantity - numberToMove)
          }]);
        }
    ...
    if (itemData.type === "component") {
      dupItem = this.items.find(it => it.name === itemData.name && it.type === itemData.type && it.system.subtype === itemData.system.subtype);
    } else {
      dupItem = this.items.find(it => it.name === itemData.name && it.type === itemData.type);
    }
    if (dupItem) {
  ...
  async handleDroppedItem(itemData) {
  ...
  async _updateDerivedDataActiveEffects() {
    ...
    for (const effect of this.effects.filter(e => !e.disabled)) {
      for (const change of effect.changes) {
        if (derivedData.includes(change.key)) {
          const changes = await effect.apply(this, change);
...
export function getWeight(item, actorData) {
...
async function getMoveNumber(itemData) {
  const returnNumber = await new Promise((resolve) => {
    new Dialog({
      title: game.i18n.localize("TWODSIX.Actor.Items.QuantityToTransfer"),
      content: `<div style="display: flex; align-items: center; gap: 2ch; justify-content: center;"><img src="` + itemData.img + `" data-tooltip = "` + itemData.name + `" width="50" height="50"> ` + itemData.name + `</div>` +
