 ...
 * @typedef {import("./_module.mjs").TwodsixCombatant} TwodsixCombatant
 */
...
export default class TwodsixCombat extends foundry.documents.Combat {
   ...
   * @param {BaseUser} user - The creating user
  ...
  getSpaceCombatConfig() {
    ...
    return TWODSIX.SPACE_COMBAT_PHASE_TYPES?.[phaseKey] || TWODSIX.SPACE_COMBAT_PHASE_TYPES.threePhase;
  }
   ...
   */
  async _updatePhaseData(phaseIndex, additionalUpdates = {}, updateOptions = {}) {
    const config = this.getSpaceCombatConfig();
    const phases = config.phases || [];
   ...
   * @returns {Promise<void>}
   */
  async _resetCombatantCounters(isNewRound) {
  ...
  async previousPhase() {
    ...
    const additionalUpdates = {turn: 0};
