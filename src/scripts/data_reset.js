...
function copy() {
  ...
  fse.copySync(srcDir, destDir + "/Data/worlds/twodsix_dev", { overwrite: true });
  fse.copySync("dist", destDir + "/Data/systems/twodsix", { overwrite: true });
  if (fse.existsSync(licensePath)) {
    fse.copySync(licensePath, destDir + "/Config/license.json", { overwrite: true });
  }
  process.exit(0);
}
...
if (fse.existsSync(srcDir)) {
  if (force) {
    ...
    rl.question(`Are you sure you want reset the world with ${srcDir}? N/y: `, function(answer) {
      if(answer === "y") {
        copy();
      } else {
        process.exit(1);
