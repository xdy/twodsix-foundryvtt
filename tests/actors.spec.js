...
const { setupTestWorld, createTestActor, shutdownWorld } = require('./helpers');

let page;

test.describe('twodsix actor types', () => {
  test.beforeAll(async ({ browser }) => {
    ({ page } = await setupTestWorld(browser, 'twodsix actor types'));
  }, 60000);
  ...
  test('create vehicle actor with full field validation', async () => {
    ...
    expect(info.hasTechLevel).toBe(true);
    // Movement is on character-type actors, not vehicle-type; this documents the current schema.
    expect(typeof info.hasMovement).toBe('boolean');
  ...
  test('create space-object actor with full field validation', async () => {
    ...
    // Movement is on character-type actors, not vehicle-type; this documents the current schema.
    expect(typeof info.hasMovement).toBe('boolean');
    // shipStats is a ShipData field; space-object extends TwodsixVehicleBaseData directly.
    expect(typeof info.hasShipStats).toBe('boolean');
  ...
  test('bulk create all 7 actor types in one evaluate call', async () => {
    const results = await page.evaluate(async () => {
      const types = ['traveller', 'animal', 'robot', 'ship', 'vehicle', 'space-object', 'world'];
      const ids = {};
      for (const type of types) {
        const actor = await Actor.create({ name: `Bulk ${type}`, type });
        ids[type] = actor.id;
      }
      ...
      for (const [type, id] of Object.entries(ids)) {
        const actor = game.actors.get(id);
        check[type] = actor ? actor.type : null;
      }
      return { ids, check };
    });

    // Each of the 7 types must have been created.
    const expectedTypes = ['traveller', 'animal', 'robot', 'ship', 'vehicle', 'space-object', 'world'];
    for (const type of expectedTypes) {
      expect(results.ids[type]).toBeTruthy();
      expect(results.check[type]).toBe(type);
    }
    expect(Object.keys(results.ids).length).toBe(7);
  });
