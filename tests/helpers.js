...
async function selectFirstAvailableUser(page) {
  ...
  for (let attempt = 0; attempt < 8; attempt++) {
    try {
      ...
      // select not ready or no options yet — reload and retry.
    }
...
async function dismissTourIfVisible(page) {
  ...
  // Foundry tours API (only works in-world, not on setup page).
  await page.evaluate(async () => {
    const activeTours = game?.tours?.activeTours;
    if (!(activeTours instanceof Map)) {
      return;
    }
  ...
  // Fallback: brute-force remove any overlay still blocking clicks (e.g. setup page tour).
  if (await page.locator('.tour-overlay').isVisible({ timeout: 500 }).catch(() => false)) {
    await page.locator('.tour-overlay, [class*="tour"][class*="overlay"]').evaluateAll((els) => {
 ...
 *
 * @param {import('@playwright/test').Page} page
 * @param {string} label - description logged with the timing result
 * @param {() => Promise<any>} fn - async operation to time
 * @returns {Promise<any>} the return value of `fn`
 */
async function timedStep(page, label, fn) {
  const start = performance.now();
  const result = await fn();
  const duration = performance.now() - start;
  console.log(`[timed] ${label}: ${duration.toFixed(0)}ms`);
  return result;
}
...
async function shutdownWorld(page) {
  ...
  try {
    ...
    // world may already be shutting down
  }
