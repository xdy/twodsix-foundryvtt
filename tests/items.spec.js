...
// Verifies key item types with schema validation, then batch-tests all 15.

const { test, expect } = require('@playwright/test');
const { setupTestWorld, createTestItem, shutdownWorld } = require('./helpers');

let page;

test.describe('twodsix item types', () => {
  test.beforeAll(async ({ browser }) => {
    ({ page } = await setupTestWorld(browser, 'twodsix item types'));
  }, 60000);
  ...
  test('create augment item with weight, quantity, and equipped fields', async () => {
    ...
    const info = await page.evaluate((id) => {
      ...
      return {
        ...
        hasWeight: 'weight' in sys,
        hasQuantity: 'quantity' in sys,
        hasEquipped: 'equipped' in sys,
      };
    }, itemId);
    ...
    expect(info.hasEquipped).toBe(true);
  });
  ...
  test('create tool item with weight and quantity fields', async () => {
    const itemId = await createTestItem(page, 'Test Tool', 'tool', {
      weight: 3,
      quantity: 1,
    });
    ...
    expect(info.weight).toBe(3);
    expect(info.quantity).toBe(1);
    expect(info.hasWeight).toBe(true);
    expect(info.hasQuantity).toBe(true);
  });

  test('create storage item with quantity and weight fields', async () => {
    const itemId = await createTestItem(page, 'Test Storage', 'storage', {
      quantity: 5,
      weight: 10,
    });
    ...
    expect(info.quantity).toBe(5);
    expect(info.weight).toBe(10);
    expect(info.hasQuantity).toBe(true);
    expect(info.hasWeight).toBe(true);
  });

  test('create junk item with quantity and weight fields', async () => {
    const itemId = await createTestItem(page, 'Test Junk', 'junk', {
      quantity: 7,
      weight: 2,
    });
    ...
    expect(info).not.toBeNull();
    expect(info.type).toBe('junk');
    expect(info.quantity).toBe(7);
