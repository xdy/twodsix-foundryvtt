//Take text block from https://travellertools.azurewebsites.net/Home/ and translate into a character
...
//Get Input
	let raw_text = await new Promise((resolve)=>{
		new Dialog({
			...
			content : `<label>Don't include menu bar (include all text with white background)</label><textarea type="text" name="input" cols="40" rows="5"></textarea>`,
			buttons : {
				 OK : { label : `Process`, 
					 callback : (html) => { 
	...
	if (processedText[line]=='') {++line;}
	
	//Process first line which is of the generic format "honortific name permalink"
	//locate name on first line
	let fullName = processedText[line].slice(0, processedText[line].indexOf(`permalink`)-1);
	...
	let upp_order = ['strength', 'dexterity', 'endurance', 'intelligence', 'education', 'socialStanding'];
	...
	for (let i= 0; i<Math.min(ageGenStats.length-2, upp_order.length); ++i){
		...
		let statVal = ageGenStats[i+2].slice(0, ageGenStats[i+2].indexOf(`(`)-1);
		
		await actor.update({ 
			[char_id] : parseInt(statVal)
		});
	}
	++line;
	...
	//add description
	await actor.update({
		'data.description': processedText[line]
	});
	...
	{
		let skillLevel = parseInt(processedText[line][processedText[line].length-1]);
		let skillName = processedText[line].slice(0, processedText[line].length-2);
	...
	while (processedText[line].indexOf('History')<0 && processedText[line].indexOf('Education')<0){
	...
	while (processedText[line].indexOf('\t')>=0){
		...
		++line;
		if ( processedText[line]== undefined){break;}
	}
	bio+=`</table><br>`;
...
function genTableRowHTML(rowText, headerText){
	if (rowText.indexOf('\t')<0){return('');}
	
	let parsedLine = rowText.split('\t');
	let returnText ='<tr>';
	for (let i=0; i<parsedLine.length; ++i){
		if (parsedLine[0].indexOf(headerText)>=0){
			returnText += '<th style="text-align:center">'+parsedLine[i].trim()+'</th>';
		}
		else {
			returnText += '<td style="text-align:center">' + parsedLine[i].trim() + '</td>';
		}
	}
	returnText+='</tr>';
	return(returnText);
