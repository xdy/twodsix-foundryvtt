//Take text block from http://members.ozemail.com.au/~jonoreita/SupplementOne/Cepheus_Engine_1001_characters.html and translate into a character
//skills must be separated by commas
...
//Get Input
	let raw_text = await new Promise((resolve)=>{
		new Dialog({
			...
			content : `<label>Must select comma separated skills</label><textarea type="text" name="input" cols="40" rows="5"></textarea>`,
			buttons : {
				 CE : { label : `Cepheus Engine`, 
					 callback : (html) => { 
						 resolve(html.find('[name="input"]')[0].value); 
						 compendium ="CE"; }
					},
				 CEL: { label : `Cepheus Light`,
					  callback : (html) => { 
						  resolve(html.find('[name="input"]')[0].value); 
						  compendium ="CL" ;}}
				 }
	...
	//Process first line which is of the generic format "#. honorific name(s) (gender, species) UPP Age #"
	
	//locate key positions on first line
	let startName = 0;
	
	if (processedText[line].indexOf('.') != -1){
		startName = processedText[line].indexOf(`.`)+2; //Adjust for numbered character
	}
	...
	let genderSpecies = processedText[line].slice(posOpenParen+1, posCloseParen).split(" ");
	...
	let cash = 0;
	let debt = 0;
	if (credits >=0){cash = credits;} else{debt = -credits;}
	...
	let upp_order = ['strength', 'dexterity', 'endurance', 'intelligence', 'education', 'socialStanding', 'psionicStrength'];
	...
	for (let i= 0; i<Math.min(upp.length, upp_order.length); ++i){
		...
		await actor.update({ 
			[char_id] : hexToBase10(upp[i])
		});
	}
	
	//generate array of skill-level pairs
	let cleanSkills = processedText[line].trim(); //get rid of extra whitespace
	if (cleanSkills[cleanSkills.length -1] ===',') { //Get rid of end of string ',' if present
		cleanSkills=cleanSkills.slice(0,-1);
	}
	let skillsList = cleanSkills.split(', '); //make an array of individual skill-level entries
	
	//Open Compendium
	let packName='';
	switch (compendium){
		case 'CE':  packName = 'twodsix.ce-srd-items'; break;
		case 'CL':  packName = 'twodsix.cepheus-light-items'; break;
		}
	...
	for (let i=0; i<skillsList.length; ++i){
		...
		let lastDash = skillsList[i].lastIndexOf('-');	//Some skills have dash in name, so last one is the marker
	...
	if (processedText[line] !=='Character Event Log:'){
		...
		for (let i=0; i<itemList.length; ++i){
			...
			if (newItem != null){await actor.createOwnedItem(newItem);}
		}
...
function translateSkillName(name){
	switch(name) {
		...
		case 'Winged Aircraft':
			switch (compendium){
				case 'CE': return('Aircraft (' + name + ')');
				case 'CL': return('Aircraft');
			}
		...
		case 'Veterinary Medicine':
			switch (compendium){
				case 'CE': return('Animals (' + name + ')');
				case 'CL': return('Animals');
			}
		...
		case 'Slug Rifle':
			switch (compendium){
				case 'CE': return('Gun Combat (' + name + ')');
				case 'CL': return('Gun Combat');
			}
		...
		case 'Turret Weapons':
			switch (compendium){
				case 'CE': return('Gunnery (' + name + ')');
				case 'CL': if(name=='Heavy Weapons'){return(name);}else{return('Gunnery');}
			}
		...
		case 'Piercing Weapons':
			switch (compendium){
				case 'CE': return('Melee Combat (' + name + ')');
				case 'CL': return('Melee');
			}
		...
		case 'Space Sciences':
			switch (compendium){
				case 'CE': return('Science (' + name + ')');
				case 'CL': return('Science');
			}
		...
		case 'Wheeled Vehicle':
			switch (compendium){
				case 'CE': return('Vehicle (' + name + ')');
				case 'CL': return('Driving');
			}
		...
		case 'Submarine':
			switch (compendium){
				case 'CE': return('Watercraft (' + name + ')');
				case 'CL': return('Watercraft');
			}
		
		case 'Battle Dress':
			switch (compendium){
				case 'CE': return(name);
				case 'CL': return('Zero-G');
			}
		case 'Gambling':
		case 'Carousing':
			switch (compendium){
				case 'CE': return(name);
				case 'CL': return('Carouse');
			}
		case 'Computer':
		case 'Comms':
			switch (compendium){
				case 'CE': return(name);
				case 'CL': return('Computers');
			}
		case 'Electronics':
		case 'Mechanics':
		case 'Gravitics':
			switch (compendium){
				case 'CE': return(name);
				case 'CL': return('Repair');
			}
		case 'Navigation':
			switch (compendium){
				case 'CE': return(name);
				case 'CL': return('Piloting');
			}
		case 'Broker':
		case 'Bribery':
			switch (compendium){
				case 'CE': return(name);
				case 'CL': return('Liaison');
			}
		case 'Advocate':
			switch (compendium){
				case 'CE': return(name);
				case 'CL': return('Admin');
			}
		default:
			return(name);
		}
...
function compendiumErrors(name){
	switch (name){
		...
		case 'Jack o\' Trades':
			switch (compendium){
				case 'CE': return('Jack of All Trades');
				case 'CL': return('Jack-of-All-Trades');
			}
