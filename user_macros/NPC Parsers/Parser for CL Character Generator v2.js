//Take text block from https://cepheuslightgen.herokuapp.com/ and translate into a Cepheus Light character
getInputText();
...
//Get Input
	let raw_text = await new Promise((resolve)=>{
		new Dialog({
			...
			content : `<label>Must select just the character block</label><textarea type="text" name="input" cols="40" rows="5"></textarea>`,
			buttons : { 
					OK : { label : `Process`, 
					 callback : (html) => { 
	...
	//break up first line which is of the generic format "honorific name(s) UPP Age #"
	...
	for (let i=0; i<skillsList.length; ++i){
		...
		if (skillItem != null){
			...
			let newSkill = await actor.items.find(item => item.data.name == skillName);
