//Macro to generated trade goods pricing and quantities for sale based on planet UPP and a trader modier for skill attributes
...
async function generateTable()
{
	...
	if (DEBUG){console.log('Trade codes:', tcodes);}
	if (DEBUG){console.log('UWP: ', uwp, traderDM);}
	
	let trade_table = ``;
	trade_table += process_tradeTable('Advanced Trade Goods - ' + compendium, tcodes, parseInt(traderDM), compendium,starBase);
	
	trade_table += process_tradeTable("Basic Goods - " + compendium, tcodes, parseInt(traderDM), compendium, starBase);
	...
	let value2 = await new Promise((resolve)=>{
		new Dialog({
			...
			buttons : {
				Ok : { label : `Ok`, callback : (html) => { resolve(html.find("input").val()); }, height:`12px`}
				}
...
function process_tradeTable(tableName, trcodes, offset, compendium, starBase){
	...
	if (DEBUG){console.log(isAvailable);}
	...
	for (let row = 0; row<table.data.results.length; ++row){
		...
		if (DEBUG){console.log('Name: ', details[0]);}
		if (DEBUG){console.log('pSellMod:', pSellMod);}
		if (DEBUG){console.log('pBuyMod:', pBuyMod);}
		...
		if(RANDOM){
			...
			if ((tableName.indexOf('Basic')!=-1)||(availableGood(trcodes, details[3]))){
				tons = new Roll("@dice", {dice: details[2]}).evaluate().total;
			}
		}
		...
		pSellPr = Math.round(details[1] * rollPriceAdjust(pSellMod+offset, "sell", compendium));
		
		//generate buy-sell table row in html
		if (row == table.data.results.length-1){
			ret_text += `<tr style="border-bottom:1px solid red"><td style="padding-right:5px">${details[0]}</td>`;
		} else {
...
function determineGoods(table, compendium, starBase) {
	...
	if (DEBUG){console.log('Number of Draws: ', numDraws);}
	
	if (numDraws<1){numDraws = 1;}
...
function getMod(planetTrCodes, goodCodes){
	...
	for (let code of planetTrCodes){ 
		...
		if (codePos!= -1){
			...
			if (codeValue > modifier){modifier = codeValue;}
		}
	}
...
function rollPriceAdjust(offset, type, compendium){
	...
	if (DEBUG){console.log('Price Adjustment Table: ', tableName);}
	
	const table = game.tables.entities.find(t => t.name === tableName);
	
	let r = new Roll("2D6+@mod", {mod: offset}).evaluate().total;
	let details = table.data.results[Math.min(Math.max(r-2,0), table.data.results.length-1)].text;
	
	if (DEBUG){console.log('Roll on Adj Table: ', r);}
	if (DEBUG){console.log('Relative Price: ', details);}
